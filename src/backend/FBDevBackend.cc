  ...
  : Backend("fbdev")
{
  ...
  if (this->fb_fd == -1) {
    Nan::ThrowError("Cannot open framebuffer device");
    return;
  }
  ...
  if (this->fb_data == MAP_FAILED) {
    Nan::ThrowError("Failed to map framebuffer device to memory");
    return;
  }
}
...
cairo_surface_t* FBDevBackend::createSurface()
{
  ...
  switch(fb_vinfo.bits_per_pixel) {
    ...
    default:
      Nan::ThrowError("Only valid formats are RGB16_565 & ARGB32");
  }
